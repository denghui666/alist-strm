...
def normalize_download_interval_range(download_interval_range, logger):
    if not isinstance(download_interval_range, (list, tuple)) or len(download_interval_range) != 2:
        logger.warning("配置中的 'download_interval_range' 无效，使用默认值 1-3。")
        return 1, 3

    try:
        min_interval = int(download_interval_range[0])
        max_interval = int(download_interval_range[1])
    except (TypeError, ValueError):
        logger.warning("下载间隔范围的值必须为整数，使用默认值 1-3。")
        return 1, 3
...
def process_with_cache(webdav, config, script_config, config_id, size_threshold, logger, min_interval=None, max_interval=None):
    ...
    if min_interval is None or max_interval is None:
        min_interval, max_interval = normalize_download_interval_range(
            config.get('download_interval_range', (1, 3)), logger
        )
    else:
        min_interval, max_interval = normalize_download_interval_range((min_interval, max_interval), logger)
